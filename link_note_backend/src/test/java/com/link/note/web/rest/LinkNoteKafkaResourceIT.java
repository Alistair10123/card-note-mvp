...
import com.link.note.config.EmbeddedKafka;
import com.link.note.config.KafkaSseConsumer;
import com.link.note.config.KafkaSseProducer;
...
class LinkNoteKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/link-note-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/link-note-kafka/consume")
