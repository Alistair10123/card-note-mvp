...
@RestController
@RequestMapping("/api/link-note-kafka")
public class LinkNoteKafkaResource {

    private final Logger log = LoggerFactory.getLogger(LinkNoteKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public LinkNoteKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
